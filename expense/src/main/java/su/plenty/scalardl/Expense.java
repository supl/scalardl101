...
public class Expense {
	public static void main(String[] args) throws Exception {
		...
		try {
			service.registerContract("expense", "su.plenty.scalardl.contract.Expense",
					"Expense.class", Optional.empty());
		} catch (Exception e) {
		}

		String date = new SimpleDateFormat("yyyyMMdd").format(new Date());
		JsonObject argument = Json.createObjectBuilder().add("date", date)
				.add("expense", Integer.parseInt(args[0])).add("log", args[1]).build();
