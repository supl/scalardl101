...
public class Withdraw extends Contract {
  ...
  public JsonObject invoke(Ledger ledger, JsonObject argument, Optional<JsonObject> property) {
    ...
    if (!asset.isPresent()) {
      throw new ContractContextException("account: " + account + " does not exist");
    }

    int balance = asset.get().data().getInt("balance");

    if (balance < amount) {
      throw new ContractContextException("balance is not enough");
    }
